...
try{
	...
	handleAPIResponse(500, 'Unable to query database.', buildPayload(['sn' => $sn]), $endPoint, $time_start);
	exit();
} catch (Exception $e) {
	// log error here
...
if ($row && $row['active']){
	...
	handleAPIResponse(200, 'Serial Number Found!', buildPayload($payload), $endPoint, $time_start);
	exit();
}
// device is not active, log output and return not found
	...
	if($row){
		...
		handleAPIResponse(200, 'Serial Number Not found', buildPayload($payload), $endPoint, $time_start);
		exit();
}
